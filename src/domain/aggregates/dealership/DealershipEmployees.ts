...
export class DealershipEmployees {
    ...
    public addEmployee(user: User): DealershipEmployees {
        if (!this.isValidEmployeeRole(user.role)) {
            throw new DealershipValidationError("Invalid user role for dealership employee");
        }

        if (this.hasEmployee(user.id)) {
            throw new DealershipValidationError("Employee already exists in this dealership");
        }
