import {
  CompanyProps,
  CreateCompanyProps,
  ReconstitueCompanyProps,
} from '@domain/interfaces/company/ICompanyProps';
...
export class Company {
  ...
  public updateContactInfo(contactInfo: ContactInfo): void {
    if (!this.props.isActive) {
      throw new CompanyValidationError(
        'Cannot update contact info of an inactive company',
      );
    }
  ...
  public updateAddress(address: Address): void {
    if (!this.props.isActive) {
      throw new CompanyValidationError(
        'Cannot update address of an inactive company',
      );
    }
  ...
  public updateName(name: string): void {
    if (!this.props.isActive) {
      throw new CompanyValidationError(
        'Cannot update name of an inactive company',
      );
    }
    if (!name.trim()) {
      throw new CompanyValidationError('Company name cannot be empty');
  ...
  public addEmployee(user: User): void {
    if (!this.props.isActive) {
      throw new CompanyValidationError(
        'Cannot add employee to an inactive company',
      );
    }
  ...
  public removeEmployee(userId: string): void {
    if (!this.props.isActive) {
      throw new CompanyValidationError(
        'Cannot remove employee from an inactive company',
      );
    }
