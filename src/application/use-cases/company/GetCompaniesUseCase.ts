...
export class GetCompaniesUseCase implements IAuthorizationAware {
    ...
    public async execute(dto: GetCompaniesDTO): Promise<Result<CompanyResponseDTO[], Error>> {
        try {
            ...
            else if (dto.userRole === UserRole.DEALERSHIP_MANAGER && dto.userDealershipId) {
                companies = await this.companyRepository.findByDealershipId(dto.userDealershipId);
            ...
            return new Error('An unexpected error occurred while retrieving companies');
        }
    }
}