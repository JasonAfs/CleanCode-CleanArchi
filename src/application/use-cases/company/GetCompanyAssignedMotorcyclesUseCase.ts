...
export class GetCompanyAssignedMotorcyclesUseCase
  implements IAuthorizationAware
{
  constructor(
  ...
  public getAuthorizationContext(
    dto: GetCompanyAssignedMotorcyclesDTO,
  ...
  public async execute(
    ...
    if (dto.includeInactive) {
      assignments = await this.companyMotorcycleRepository.findByCompanyId(
        dto.companyId,
      );
    } else {
      assignments =
    ...
    return motorcycles.filter(
      (motorcycle): motorcycle is Motorcycle => motorcycle !== null,
    );
  }
}
