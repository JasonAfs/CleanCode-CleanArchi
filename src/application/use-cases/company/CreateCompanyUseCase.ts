...
export class CreateCompanyUseCase implements IAuthorizationAware {
    ...
    public async execute(dto: CreateCompanyDTO): Promise<Result<CompanyResponseDTO, Error>> {
        try {
            ...
            if (exists) {
                return new CompanyValidationError(`Company with registration number ${dto.registrationNumber} already exists`);
            }
            ...
            return new Error('An unexpected error occurred while creating the company');
        }
    }
}