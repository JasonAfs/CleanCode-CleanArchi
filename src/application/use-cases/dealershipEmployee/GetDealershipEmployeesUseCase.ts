    ...
    public async execute(
        ...
        try {
            ...
            if (!userPermissions?.has(Permission.VIEW_DEALERSHIP_DETAILS)) {
                return new UnauthorizedError("You don't have permission to view dealership employees");
            }

            // 2. Pour les managers de concession, vérifier qu'ils accèdent à leur propre concession
            if (dto.userRole === UserRole.DEALERSHIP_MANAGER && dto.dealershipId !== dto.userDealershipId) {
                return new UnauthorizedError("You don't have access to this dealership's employees");
            }
            ...
            if (dto.roleFilter) {
                employees = employees.filter(employee => employee.role === dto.roleFilter);
            }
            ...
            return new Error('An unexpected error occurred while retrieving dealership employees');
        }
    }
}