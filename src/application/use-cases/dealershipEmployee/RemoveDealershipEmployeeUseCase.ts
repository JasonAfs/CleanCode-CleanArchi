...
export class RemoveDealershipEmployeeUseCase implements IAuthorizationAware {
  ...
  public getAuthorizationContext(
    dto: RemoveDealershipEmployeeDTO,
  ...
  public async execute(
    dto: RemoveDealershipEmployeeDTO,
  ): Promise<Result<RemoveDealershipEmployeeResponseDTO, Error>> {
    try {
      ...
      if (!dealership.hasEmployee(dto.employeeId)) {
        return new DealershipValidationError(
          'Employee is not associated with this dealership',
        );
      }

      // Étape 6: Vérification des règles métier pour les managers
      if (employee.role === UserRole.DEALERSHIP_MANAGER) {
        const remainingManagers = dealership.employees
          .getByRole(UserRole.DEALERSHIP_MANAGER)
          .filter((manager) => manager.id !== dto.employeeId);

        if (remainingManagers.length === 0) {
          return new DealershipValidationError(
            'Cannot remove the last manager of a dealership',
          );
        }
      }
      ...
      return new Error(
        'An unexpected error occurred while removing employee from dealership',
      );
    }
  }
}
