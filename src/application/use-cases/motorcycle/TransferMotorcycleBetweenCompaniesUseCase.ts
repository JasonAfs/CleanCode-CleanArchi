...
export class TransferMotorcycleBetweenCompaniesUseCase {
  private readonly validator =
    new TransferMotorcycleBetweenCompaniesValidator();
    ...
    try {
      ...
      if (!targetCompany) {
        return new CompanyValidationError(
          `Target company not found with id: ${dto.targetCompanyId}`,
        );
      }
      ...
      if (dto.userRole === UserRole.DEALERSHIP_MANAGER) {
        ...
        if (motorcycle.dealershipId !== dto.dealershipId) {
          return new UnauthorizedError(
            "You don't have access to transfer this motorcycle",
          );
        }
        ...
        if (!targetCompany.belongsToDealership(dto.dealershipId)) {
          return new UnauthorizedError(
            "You don't have access to transfer motorcycles to this company",
          );
        }
      }
      ...
      return new Error(
        'An unexpected error occurred while transferring the motorcycle between companies',
      );
    }
  }
}
