    ...
    public async execute(
        ...
        try {
            ...
            if (![UserRole.TRIUMPH_ADMIN, UserRole.DEALERSHIP_MANAGER].includes(dto.userRole)) {
                return new UnauthorizedError("Only TRIUMPH_ADMIN or DEALERSHIP_MANAGER can assign motorcycles to companies");
            }
            ...
            if (dto.userRole === UserRole.DEALERSHIP_MANAGER) {
                ...
                if (motorcycle.dealershipId !== dto.dealershipId) {
                    return new UnauthorizedError("You don't have access to assign this motorcycle");
                }

                // Vérifier que la company est gérée par le dealership du manager
                if (!company.belongsToDealership(dto.dealershipId)) {
                    return new UnauthorizedError("You don't have access to assign motorcycles to this company");
                }
            }
            ...
            return new Error('An unexpected error occurred while assigning the motorcycle to company');
        }
    }
}