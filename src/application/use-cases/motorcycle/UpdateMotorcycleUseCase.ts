    ...
    public async execute(
        ...
        try {
            ...
            if (dto.userRole !== UserRole.TRIUMPH_ADMIN) {
                return new UnauthorizedError("Only TRIUMPH_ADMIN can update motorcycles");
            }
            ...
            if (!motorcycle) {
                return new MotorcycleNotFoundError(`Motorcycle with id ${dto.motorcycleId} not found`);
            }
            ...
            if (dto.vin) {
                ...
                const existingMotorcycle = await this.motorcycleRepository.findByVin(newVin);
                if (existingMotorcycle && existingMotorcycle.id !== motorcycle.id) {
                    return new MotorcycleValidationError(`Motorcycle with VIN ${dto.vin} already exists`);
                }
            }
            ...
            return new Error('An unexpected error occurred while updating the motorcycle');
        }
    }
}