    ...
    public async execute(
        ...
        try {
            ...
            if (dto.userRole !== UserRole.TRIUMPH_ADMIN) {
                return new UnauthorizedError("Only TRIUMPH_ADMIN can transfer motorcycles");
            }
            ...
            const targetDealership = await this.dealershipRepository.findById(dto.targetDealershipId);
            if (!targetDealership) {
            ...
            if (!targetDealership.isActive) {
                return new DealershipValidationError('Cannot transfer motorcycle to an inactive dealership');
            }
            ...
            return new Error('An unexpected error occurred while transferring the motorcycle');
        }
    }
}