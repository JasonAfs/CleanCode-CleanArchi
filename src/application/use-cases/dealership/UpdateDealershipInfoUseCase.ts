...
export class UpdateDealershipInfoUseCase implements IAuthorizationAware {
  ...
  public async execute(
    dto: UpdateDealershipInfoDTO,
  ): Promise<Result<UpdateDealershipInfoResponseDTO, Error>> {
    try {
      ...
      if (this.hasContactInfoChanges(dto)) {
        const newContactInfo = ContactInfo.create(
          ...
          dto.email
            ...
            : dealership.contactInfo.emailAddress,
        );
      ...
      return new Error(
        'An unexpected error occurred while updating dealership information',
      );
    }
  }
