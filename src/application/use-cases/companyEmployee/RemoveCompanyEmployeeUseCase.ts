...
export class RemoveCompanyEmployeeUseCase implements IAuthorizationAware {
  ...
  public async execute(
    dto: RemoveCompanyEmployeeDTO,
  ): Promise<Result<RemoveCompanyEmployeeResponseDTO, Error>> {
    try {
      ...
      if (!company.hasEmployee(dto.employeeId)) {
        return new CompanyValidationError(
          'Employee is not associated with this company',
        );
      }

      // Étape 6: Vérification des règles métier pour les managers
      if (employee.role === UserRole.COMPANY_MANAGER) {
        const remainingManagers = company.employees
          .getByRole(UserRole.COMPANY_MANAGER)
      ...
      return new Error(
        'An unexpected error occurred while removing employee from company',
      );
    }
  }
}
