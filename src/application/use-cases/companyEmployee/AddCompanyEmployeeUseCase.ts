...
export class AddCompanyEmployeeUseCase implements IAuthorizationAware {
    ...
    public getAuthorizationContext(dto: AddCompanyEmployeeDTO): AuthorizationContext {
        return {
            userId: dto.userId,
    ...
    public async execute(dto: AddCompanyEmployeeDTO): Promise<Result<AddCompanyEmployeeResponseDTO, Error>> {
        try {
        ...
        if (dto.userRole !== UserRole.TRIUMPH_ADMIN && 
            ...
            return new UnauthorizedError("You don't have access to manage this company's employees");
        }
            ...
            const existingCompany = await this.companyRepository.findByEmployeeId(dto.employeeId);
            ...
            return new Error('An unexpected error occurred while adding employee to company');
        }
    }
}