...
export class UpdateDealershipInfoValidator {
  public validate(dto: UpdateDealershipInfoDTO): void {
    ...
    const hasPartialAddress = addressFields.some(
      (field) => field !== undefined,
    );
    const hasMissingAddress = addressFields.some(
      (field) => field === undefined || !field.trim(),
    );

    if (hasPartialAddress && hasMissingAddress) {
      throw new DealershipValidationError(
        'All address fields are required when updating address',
      );
    }
    ...
    if (
      dto.phone &&
      !/^[+]?[(]?[0-9]{3}[)]?[-\s.]?[0-9]{3}[-\s.]?[0-9]{4,6}$/.test(dto.phone)
    ) {
